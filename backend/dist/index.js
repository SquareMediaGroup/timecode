...
function createDrivers(settings) {
    const map = new Map();
...
function getDeviceStatuses(drivers) {
    const statuses = [];
...
async function main() {
    ...
    function broadcastCueList() {
        broadcast('cueList', cueManager.getCues());
    }
    function broadcastDeviceStatus() {
        const update = { devices: getDeviceStatuses(drivers) };
    ...
    io.on('connection', (socket) => {
        ...
        socket.emit('deviceStatus', { devices: getDeviceStatuses(drivers) });
        // Transport commands
        socket.on('transportCommand', (data) => {
            switch (data.command) {
                case 'play':
                    generator.play();
                    break;
        ...
        // Manual timecode set
        socket.on('setTimecode', (timecode) => {
            generator.setTimecode(timecode);
        });
    });
    function buildTimecodeUpdate() {
