...
export class AtemDriver extends EventEmitter {
  ...
  private sessionId = 0;
  private localPacketCounter = 0;
  private remotePacketCounter = 0;
  ...
  triggerCue(action: AtemAction): void {
    if (!action.enabled || !this.connected) return;
  ...
  private buildHeader(flags: number, payloadLen: number): Buffer {
  ...
  private sendCommand(cmdName: string, payload: Buffer): void {
  ...
  private sendCut(inputSource: number): void {
  ...
  private sendAuto(inputSource: number): void {
  ...
  private sendMacroRun(macroIndex: number): void {
