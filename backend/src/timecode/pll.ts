...
export function framesToTimecode(totalFrames: number, frameRate: FrameRate): Timecode {
...
function formatTimecode(tc: Timecode, frameRate: FrameRate): string {
...
export class PLLFilter extends EventEmitter {
  ...
  private freeRunTimer: ReturnType<typeof setInterval> | null = null;
  ...
  private startDropoutWatch(): void {
    ...
    if (this.freeRunTimer) return;

    const effectiveFps =
      this.currentFps === 29.97 ? 30000 / 1001 : this.currentFps;
